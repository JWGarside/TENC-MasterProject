...
async function comparePaperAndCode(pdfPath, codePath) {
    ...
    // Second prompt
    const second_system_instructions = `You are a helpful assistant
    ...
    console.log(`Analysing paper against code file with few-shot examples`);
...
/**
 * Filter out discrepancies that don't affect results
 * 
 * @param {string} initialAnalysis - Claude's initial response with all discrepancies
 * @returns {Promise<string>} - Filtered response with only result-affecting discrepancies
 */
async function filterDiscrepancies(initialAnalysis) {
    // Initialize the Anthropic client
    const anthropic = new Anthropic({
    ...
    const filterSystemPrompt = `You are a research reproducibility analyzer that filters analysis results.
    
    You will receive an analysis that identifies discrepancies between a research paper and its code implementation.
    
    Your task is to filter this analysis to ONLY include discrepancies marked as "Yes" in the "Affects Results?" category.
    Keep all other formatting exactly the same, including section headers and general structure. Renumber all discrepancies sequentially (Discrepancy 1, Discrepancy 2, etc.) without gaps in numbering.
    
    If NO discrepancies affect results, or there are no discrepancies, DO NOT edit the analysis. Instead, return the analysis without filtering it.`;
    
    // User prompt for filtering
    const filterPrompt = `Please filter the following analysis to only include discrepancies that affect results (marked as "Yes" in the "Affects Results?" category). 
    
    Maintain the exact same formatting and structure, just remove the discrepancies that don't affect results. Renumber all discrepancies sequentially (Discrepancy 1, Discrepancy 2, etc.) without gaps in numbering.
    
    Here's the analysis to filter:
    
    ${initialAnalysis}`;

    console.log(`Filtering discrepancies`);
    
    try {
        // Send request to Claude API
        const response = await anthropic.messages.create({
            model: "claude-3-7-sonnet-20250219", // Same model as first call
            max_tokens: 1024,
            temperature: 0.2,
        ...
        // Return Claude's filtered response
        return response.content[0].text;
    } catch (error) {
        console.error('Error filtering discrepancies:', error);
        if (error.response) {
            console.error('API Error Details:', error.response.data);
        }
        throw error;
...
app.post('/api/compare', upload.fields([
    ...
    try {
        ...
        // Run the initial comparison
        const initialAnalysis = await comparePaperAndCode(paperPath, codePath);
        
        // Filter the results to only include discrepancies that affect results
        const filteredAnalysis = await filterDiscrepancies(initialAnalysis);
